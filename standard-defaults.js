...
const STANDARD_LABEL_PATTERN = '(?:职业健康安全管理体系|质量管理体系|环境管理体系|OHSMS|QMS|EMS|[QES](?:\\s*[QES]){0,2})(?:\\s*[/、,，+&]\\s*(?:职业健康安全管理体系|质量管理体系|环境管理体系|OHSMS|QMS|EMS|[QES](?:\\s*[QES]){0,2}))*';

function splitSystemValues(value) {
  const text=String(value || ''), result={};
  const labels=[...text.matchAll(new RegExp('('+STANDARD_LABEL_PATTERN+')\\s*[:：]','gi'))];
  labels.forEach((match,i)=>{
    const label=match[1].toUpperCase();
    const tokens=label.match(/职业健康安全管理体系|质量管理体系|环境管理体系|OHSMS|QMS|EMS|[QES]/g) || [];
    const systems=Object.keys(STANDARD_SYSTEMS).filter(s=>tokens.some(t=>[s,STANDARD_SYSTEMS[s].name,STANDARD_SYSTEMS[s].code].includes(t)));
...
function splitStandardCriteria(value, systems) {
  ...
  if (Object.keys(labelled).length) return Object.fromEntries(Object.entries(labelled).map(([system,part])=>{
    const standards=splitStandardCriteria(part,[]);
    return [system,Object.keys(standards).length>1 ? standards[system] || part : part];
  }));
  const result={};
  ...
  if (systems.length===1 && (Object.keys(result).length===0 || (Object.keys(result).length===1 && result[systems[0]]))) return {[systems[0]]:text};
  return result;
}
